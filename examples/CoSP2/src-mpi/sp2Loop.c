...
#if _CD
	CDErrT err= kOK;	
	CDHandle* cdh_child = cd::GetCurrentCD()->Create("sp2Loop", kStrict, 0, 0, &err);
	CD_Begin(cdh_child);
	cdh_child->Preserve(xmatrix, sizeof(*xmatrix), kCopy, "SparseMatrixSt", 0, 0, 0, kUnsure);
	cdh_child->Preserve(domain, sizeof(*domain), kCopy, "DomainSt", 0, 0, 0, kUnsure);
#endif
...
#if _CD
	err= kOK;	
	CDHandle* cdh_child1 = cd::GetCurrentCD()->Create("sp2Loop", kStrict, 0, 0, &err);
#endif
  while ( breakLoop == 0 && iter < 100 )
  {
#if _CD
	CD_Begin(cdh_child1);
	cdh_child1->Preserve(xmatrix, sizeof(*xmatrix), kCopy, "SparseMatrixSt", 0, 0, 0, kUnsure);
	cdh_child1->Preserve(x2matrix, sizeof(*x2matrix), kCopy, "SparseMatrixSt", 0, 0, 0, kUnsure);
	cdh_child1->Preserve(domain, sizeof(*domain), kCopy, "DomainSt", 0, 0, 0, kUnsure);
	cdh_child1->Preserve(&occ, sizeof(occ), kCopy, "occ", 0, 0, 0, kUnsure);
	cdh_child1->Preserve(&idempErr, sizeof(idempErr), kCopy, "idempErr", 0, 0, 0, kUnsure);
	cdh_child1->Preserve(&idempErr1, sizeof(idempErr1), kCopy, "idempErr1", 0, 0, 0, kUnsure);
	cdh_child1->Preserve(&idempErr2, sizeof(idempErr2), kCopy, "idempErr2", 0, 0, 0, kUnsure);
#endif
    trX = ZERO;
    trX2 = ZERO;
