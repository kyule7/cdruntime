...
#if _CD
  // Initialize root CD before parallel execution
  /* //CD_profiler_on(true); */
  CDHandle* root_cd = CD_Init(nRanks, myRank);
  //CD_profiler_print("Start initialization");
  root_cd->Begin();
  root_cd->Preserve(&argc, sizeof(argc), kCopy, "argc", 0, 0, 0, kUnsure);
  //root_cd->Preserve(argv, sizeof(*argv), kCopy, "argv", 0, 0, 0, kUnsure);
#endif
