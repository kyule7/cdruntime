...
/// | \--potType    | -t          | funcfl        | potential type (funcfl or
/// setfl)
/// | \--doeam      | -e          | N/A           | compute eam potentials
/// (default is LJ)
/// | \--nx         | -x          | 20            | number of unit cells in x
/// | \--ny         | -y          | 20            | number of unit cells in y
/// | \--nz         | -z          | 20            | number of unit cells in z
/// | \--xproc      | -i          | 1             | number of ranks in x
/// direction
/// | \--yproc      | -j          | 1             | number of ranks in y
/// direction
/// | \--zproc      | -k          | 1             | number of ranks in z
/// direction
/// | \--nSteps     | -N          | 100           | total number of time steps
/// | \--printRate  | -n          | 10            | number of steps between
/// output
/// | \--dt         | -D          | 1             | time step (in fs)
/// | \--lat        | -l          | -1            | lattice parameter
/// (Angstroms)
...
///
///     $ mpirun -np 8 ../bin/CoMD-mpi -i2 -j2 -k2 -x 40 -y 40 -z 40 -N 10000 -n
///     100
///
...
#if _CD1
  addArg("preserveRate", '1', 1, 'i', &(cmd.preserveRateLevel1), 0,
         "interval for level 1 CD");
#endif
#if _CD2
  addArg("preserveRate", '2', 1, 'i', &(cmd.preserveRateLevel2), 0,
         "interval for level 2 CD");
#endif
#if _CD3
