...
static InterpolationObject *initInterpolationObject(int n, real_t x0, real_t dx,
                                                    real_t *data);
static void destroyInterpolationObject(InterpolationObject **table);
static void interpolate(InterpolationObject *table, real_t r, real_t *f,
                        real_t *df);
...
static void eamReadSetfl(EamPotential *pot, const char *dir,
                         const char *potName);
static void eamReadFuncfl(EamPotential *pot, const char *dir,
                          const char *potName);
    ...
    for (int jTmp = 0; jTmp < nNbrBoxes; jTmp++) {
      ...
      for (int iOff = MAXATOMS * iBox, ii = 0; ii < nIBox; ii++, iOff++) {
        ...
        for (int jOff = MAXATOMS * jBox, ij = 0; ij < nJBox; ij++, jOff++) {
          ...
          for (int k = 0; k < 3; k++) {
            s->atoms->f[iOff][k] -=
                (pot->dfEmbed[iOff] + pot->dfEmbed[jOff]) * dRho * dr[k] / r;
            s->atoms->f[jOff][k] +=
                (pot->dfEmbed[iOff] + pot->dfEmbed[jOff]) * dRho * dr[k] / r;
          }
...
InterpolationObject *initInterpolationObject(int n, real_t x0, real_t dx,
                                             real_t *data) {
