...
void serprvHaloExchange(HaloExchange* hh)
{
  ...
  cd_preserve(cdh, hh->nbrRank, 6*sizeof(int), kCopy, "HaloExchange_nbrRank", NULL);
  cd_preserve(cdh, &(hh->bufCapacity), sizeof(int), kCopy, "HaloExchange_bufCapacity", NULL);

  cd_preserve(cdh, hh->loadBuffer, sizeof(&loadAtomsBuffer), kCopy, "HaloExchange_loadBuffer", NULL);
  cd_preserve(cdh, hh->unloadBuffer, sizeof(&unloadAtomsBuffer), kCopy, "HaloExchange_loadBuffer", NULL);
  cd_preserve(cdh, hh->destroy, sizeof(&destroyAtomsExchange), kCopy, "HaloExchange_destroy", NULL);

  // FIXME: parms is cased into both AtomExchangeParms and ForceExchangeParams
  AtomExchangeParms* parms = hh->parms;
  cd_preserve(cdh, parms, sizeof(AtomExchangeParms*), kCopy, "HaloExchange_parms", NULL);
  cd_preserve(cdh, parms->nCells, 6*sizeof(int), kCopy, "HaloExchange_parms_nCells", NULL);
  cd_preserve(cdh, parms->cellList[0], parms->nCells[0]*sizeof(int), kCopy, "HaloExchange_parms_cellList0", NULL);
  cd_preserve(cdh, parms->cellList[1], parms->nCells[1]*sizeof(int), kCopy, "HaloExchange_parms_cellList1", NULL);
  cd_preserve(cdh, parms->cellList[2], parms->nCells[1]*sizeof(int), kCopy, "HaloExchange_parms_cellList2", NULL);
  cd_preserve(cdh, parms->cellList[3], parms->nCells[2]*sizeof(int), kCopy, "HaloExchange_parms_cellList3", NULL);
  cd_preserve(cdh, parms->cellList[4], parms->nCells[3]*sizeof(int), kCopy, "HaloExchange_parms_cellList4", NULL);
  cd_preserve(cdh, parms->cellList[5], parms->nCells[4]*sizeof(int), kCopy, "HaloExchange_parms_cellList5", NULL);
  cd_preserve(cdh, parms->pbcFactor[0], 3*sizeof(real_t), kCopy, "HaloExchange_parms_pbcFactor0", NULL);
  cd_preserve(cdh, parms->pbcFactor[1], 3*sizeof(real_t), kCopy, "HaloExchange_parms_pbcFactor1", NULL);
  cd_preserve(cdh, parms->pbcFactor[2], 3*sizeof(real_t), kCopy, "HaloExchange_parms_pbcFactor2", NULL);
  cd_preserve(cdh, parms->pbcFactor[3], 3*sizeof(real_t), kCopy, "HaloExchange_parms_pbcFactor3", NULL);
  cd_preserve(cdh, parms->pbcFactor[4], 3*sizeof(real_t), kCopy, "HaloExchange_parms_pbcFactor4", NULL);
  cd_preserve(cdh, parms->pbcFactor[5], 3*sizeof(real_t), kCopy, "HaloExchange_parms_pbcFactor5", NULL);
