...
typedef struct HaloExchangeSt {
  ...
  void (*unloadBuffer)(void *parms, void *data, int face, int bufSize,
                       char *buf);
...
HaloExchange *initAtomHaloExchange(struct DomainSt *domain,
                                   struct LinkCellSt *boxes);

/// Create a HaloExchange for force data.
HaloExchange *initForceHaloExchange(struct DomainSt *domain,
                                    struct LinkCellSt *boxes);
