...
static void copyAtom(LinkCell *boxes, Atoms *atoms, int iAtom, int iBox,
                     int jAtom, int jBox);
...
LinkCell *initLinkCells(const Domain *domain, real_t cutoff) {
  ...
  ll->nHaloBoxes =
      2 * ((ll->gridSize[0] + 2) * (ll->gridSize[1] + ll->gridSize[2] + 2) +
  ...
  assert((ll->gridSize[0] >= 2) && (ll->gridSize[1] >= 2) &&
         (ll->gridSize[2] >= 2));
  return ll;
}
...
void putAtomInBox(LinkCell *boxes, Atoms *atoms, const int gid, const int iType,
...
int getBoxFromTuple(LinkCell *boxes, int ix, int iy, int iz) {
  ...
  // Halo in X+
    ...
    iBox =
        boxes->nLocalBoxes + gridSize[1] * gridSize[2] + iz * gridSize[1] + iy;
  }
  // Halo in X-
  else if (ix == -1) {
...
void copyAtom(LinkCell *boxes, Atoms *atoms, int iAtom, int iBox, int jAtom,
              int jBox) {
